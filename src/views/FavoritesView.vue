  ...
  <div class="flex flex-col h-full overflow-y-auto custom__scrollbar">
    ...
    <div class="flex-1 px-4 lg:px-8 py-6">
      ...
      >
        ...
        <p class="text-gray-400 dark:text-gray-500 text-sm font-medium text-center">
          {{ ui.t('noFavorites') }}
        </p>
        <RouterLink
          to="/shop"
      ...
      <div
        v-else
        class="flex flex-col gap-3 max-w-2xl mx-auto lg:mx-0"
      >
        <div
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="text-[10px] font-bold text-brand uppercase tracking-wider mb-0.5">
              {{ product.brand }}
            </p>
            <RouterLink
              :to="`/product/${product.id}`"
              class="text-sm font-semibold text-obsidian dark:text-white hover:text-brand line-clamp-2 leading-snug block"
            >
              {{ product.name }}
            </RouterLink>
            ...
            <div class="flex items-baseline gap-1.5 mt-1.5">
              ...
              <span
                v-if="product.discountPercentage > 0"
                class="text-[10px] font-bold text-white bg-brand px-1.5 py-0.5 rounded-full"
              >
