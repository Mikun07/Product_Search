<template>
  <div class="flex flex-col lg:flex-row h-full overflow-y-auto custom__scrollbar">
    <template v-if="product">
      ...
      <div class="relative lg:w-1/2 lg:h-full h-72 bg-gray-100 dark:bg-obsidian-3 shrink-0">
        ...
        <button
          ...
          @click="router.back()"
        >←</button>
        <span v-if="product.discountPercentage > 0" class="absolute top-4 right-4 bg-brand text-white text-xs font-bold px-3 py-1 rounded-full">
      ...
      <div class="flex-1 flex flex-col gap-6 p-6 lg:p-10 overflow-y-auto custom__scrollbar">
        <p class="text-xs font-semibold text-brand uppercase tracking-widest">{{ product.brand }}</p>

        <div class="flex items-start justify-between gap-4">
          <h1 class="text-xl lg:text-2xl font-bold text-obsidian dark:text-white leading-tight">{{ product.name }}</h1>
          <button
        ...
        <div class="flex items-center gap-2">
          <div class="flex items-center gap-0.5">
            <span v-for="i in 5" :key="i" :style="i <= Math.round(product.rating) ? 'color:#f59e0b' : 'color:#d1d5db'">★</span>
          </div>
        ...
        <div class="flex items-baseline gap-3">
          ...
          <span v-if="effectivePrice" class="text-base text-gray-400 line-through">{{ ui.format(product.price) }}</span>
        </div>

        <hr class="border-gray-100 dark:border-obsidian-3" />

        <div>
          <h2 class="text-sm font-bold text-obsidian dark:text-white mb-2">{{ ui.t('description') }}</h2>
        ...
        <div class="flex items-center gap-3 mt-auto">
          ...
          >
            🛒 {{ ui.t('addToCart') }}
            <span v-if="cartItem" class="bg-white/20 text-white text-xs px-1.5 py-0.5 rounded-full">
    ...
    <div v-else class="flex flex-col items-center justify-center h-full gap-4 text-gray-400 dark:text-gray-500">
      <p class="text-lg">{{ ui.t('notFound') }}</p>
      <RouterLink to="/shop" class="text-sm text-brand hover:underline font-semibold">← {{ ui.t('back') }}</RouterLink>
    </div>
  </div>
</template>
