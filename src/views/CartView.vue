<template>
  <div
    ...
    class="flex flex-col items-center justify-center h-full gap-5 px-4"
  >
    ...
    <RouterLink
      to="/shop"
      class="px-6 py-2.5 bg-brand hover:bg-brand-dark text-white rounded-xl font-semibold text-sm transition-colors"
    >
  ...
  <div
    v-else
    class="flex flex-col lg:flex-row h-full overflow-y-auto custom__scrollbar gap-0"
  >
    <!-- Items list -->
        ...
        >
          ...
          >
            <img
              :src="item.image"
              :alt="item.name"
              class="h-16 w-16 rounded-xl object-cover bg-gray-100 dark:bg-obsidian-3"
            >
          </RouterLink>
          <div class="flex-1 min-w-0">
            <RouterLink
              :to="`/product/${item.id}`"
              class="text-sm font-semibold text-obsidian dark:text-white hover:text-brand line-clamp-2 leading-tight block"
            >
              {{ item.name }}
            </RouterLink>
            ...
            <div class="flex items-baseline gap-1.5 mt-1">
              ...
              >{{ ui.format(item.price) }}</span>
            </div>
          </div>
          <!-- Quantity -->
          <div class="flex items-center gap-1.5 bg-gray-100 dark:bg-obsidian-3 rounded-xl px-2 py-1.5 shrink-0">
            <button
              class="h-6 w-6 flex items-center justify-center rounded-lg hover:bg-white dark:hover:bg-obsidian-2 text-obsidian dark:text-white transition-colors"
    ...
    <div class="lg:w-80 lg:shrink-0 px-4 lg:px-6 py-6 bg-white dark:bg-obsidian-2 border-t lg:border-t-0 lg:border-l border-gray-100 dark:border-obsidian-3">
      <h2 class="text-sm font-bold text-obsidian dark:text-white mb-4">
        Order Summary
      </h2>
